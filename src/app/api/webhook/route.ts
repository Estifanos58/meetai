...
function verifySignatureWithSDK(body: string, signature: string): boolean {
  // This function should implement the actual signature verification logic
  // using the SDK's method for verifying webhook signatures.
    ...
    const [existingMeeting] = await db
      ...
      .where(
        and(
          ...
          not(eq(meetings.status, "complete")),
          not(eq(meetings.status, "active")),
          not(eq(meetings.status, "cancelled")),
          not(eq(meetings.status, "processing")) // Ensure the meeting is not deleted
        )
        );
